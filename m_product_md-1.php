...
$img=$_FILES['updateimg']['name'];
if($_FILES['updateimg']['error']==0){
	move_uploaded_file($_FILES['updateimg']['tmp_name'],"productsimages/".$_FILES['updateimg']['name']);}
	elseif($_FILES['updateimg']['error']==4){$img=$_POST['productimages'];}
	else{echo '上傳檔案失敗,請聯絡管理員';}
  $updateSQL = "UPDATE product SET productname='$productname', productprice='$productprice', productimages='$productimages', `description`='$pdescription', a1='$a1', a2='$a2', a3='$a3' WHERE productid='$productid'";
...
<form name="form1" method="POST" action="<?php echo $editFormAction; ?>" enctype="multipart/form-data">
  ...
  <img src="productimages/<?php echo $row_Recordset1['productimages']; ?>" width="250" alt="">
  <input name="productimages" type="hidden" id="productimages" value="<?php echo $row_Recordset1['productimages']; ?>">
  </p>
  <!--<p>&nbsp;<img src="icon_prev.gif" alt="這是顯示上傳預覽圖片的位置" name="showImg" id="showImg" onClick='javascript:alert("這是顯示上傳預覽圖片的位置");'>
