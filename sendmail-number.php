...
header("Content-Type:text/html; charset-utf-8");
    ...
    $sql="select CustomerEmail from orders where OrderID=:OrderID";
    $stmt=$link->prepare($sql);
    $stmt->bindPARAM(":OrderID", $OrderID, PDO::PARAM_STR);
    $stmt->execute();
    $result=$stmt->fetchAll();
    // $id = $result[0]["OrderID"];//陣列
...
$to = $result[0]["CustomerEmail"];
$subject = mb_encode_mimeheader('【快樂芒果園】訂單編號','UTF-8');
$message = "您的訂單編號為：".$_POST['id'];
